...
def serverThree():
	with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sc1:
		...
		while x < 6:
			...
			cursor.execute(" INSERT INTO s06m2(dtime, v_res, cb_ctrl, cb_res, ld_res, f_res, hv_p_res, hv_q_res, lv_p_res, lv_q_res, tr_tap, tr_tap_ctrl, tr_tap_mode, tr_tap_res) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", inserted_values)
